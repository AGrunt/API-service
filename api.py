...
def put_responses(id):
    ...
    try:
        for response in request.json['responses']:
            ...
            cursor.execute(insert_stmt, {'userId': id, 'questionId': response['questionId'], 'questionValue': response['value'], 'responseTimeStamp': datetime.now()})
        dbConnection.commit()
        cursor.close()
        dbConnection.close()
        return 'Created', 201
...
def put_ranking(id, cafeid):
    ...
    try:
        for ranking in request.json['rankings']:
            ...
            cursor.execute(insert_stmt, {'userId': id, 'cafeId': cafeid, 'categoryId': ranking['categoryId'], 'rankingValue': ranking['rank'], 'rankingTimeStamp': datetime.now()})
        dbConnection.commit()
        cursor.close()
        dbConnection.close()
        return 'Created', 201
